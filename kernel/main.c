...
__attribute__((used, section(".limine_requests"))) static volatile struct limine_memmap_request memmap_request = { .id = LIMINE_MEMMAP_REQUEST_ID, .revision = 0, .response = 0 };
__attribute__((used, section(".limine_requests"))) static volatile struct limine_hhdm_request hhdm_request = { .id = LIMINE_HHDM_REQUEST_ID, .revision = 0, .response = 0 };
...
__attribute__((noreturn)) void _start(void) {
    ...
    if (!LIMINE_BASE_REVISION_SUPPORTED(base_revision) || !memmap_request.response || !hhdm_request.response) {
