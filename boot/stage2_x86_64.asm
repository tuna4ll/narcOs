...
FILE_BACKED_LIMIT equ 0x0B8000
...
BOOT_FLAG_LONG_MODE equ 0x00000010
PM16_CODE_SEG equ 0x20
...
stage2_main:
    ...
    or dword [boot_flags], BOOT_FLAG_LONG_MODE
...
load_kernel:
    ...
    mov byte [kernel_error_code], 0x10
    mov eax, [kernel_lba]
    ...
    jne .err
    cmp byte [ELF_HEADER_ADDR + 4], 2
    jne .err
    cmp byte [ELF_HEADER_ADDR + 5], 1
    jne .err
    cmp word [ELF_HEADER_ADDR + 16], 2
    jne .err
    cmp word [ELF_HEADER_ADDR + 18], 0x3E
    jne .err
    cmp dword [ELF_HEADER_ADDR + 20], 1
    jne .err
    cmp dword [ELF_HEADER_ADDR + 28], 0
    jne .err
    mov eax, [ELF_HEADER_ADDR + 24]
    mov [kernel_entry], eax
    cmp dword [ELF_HEADER_ADDR + 36], 0
    jne .err
    cmp word [ELF_HEADER_ADDR + 54], ELF_PHENTSIZE64
    jne .err

    mov byte [kernel_error_code], 0x20
    mov eax, [ELF_HEADER_ADDR + 32]
    mov ebx, eax
    movzx ecx, word [ELF_HEADER_ADDR + 56]
    mov edx, ecx
    imul edx, ELF_PHENTSIZE64
    add edx, ebx
...
.ph_loop:
    ...
    mov byte [kernel_error_code], 0x30
    cmp dword [si + 12], 0
    jne .err
    cmp dword [si + 28], 0
    jne .err
...
.end_ok:
    ...
    jb .err
    je .no_zero_range
    mov byte [kernel_error_code], 0x34
    sub eax, [si + 32]
    mov ebx, [si + 24]
    add ebx, [si + 32]
    call add_zero_range
.no_zero_range:
    mov byte [kernel_error_code], 0x40
    mov eax, [si + 32]
    or eax, eax
    jz .next_ph
    mov byte [kernel_error_code], 0x41
    mov edx, [si + 8]
    add edx, eax
    jc .err
    mov byte [kernel_error_code], 0x42
    cmp edx, [kernel_file_size]
    ja .err
    mov byte [kernel_error_code], 0x43
    mov edx, eax
    add edx, [si + 24]
    jc .err
    cmp edx, FILE_BACKED_LIMIT
    ja .err
    mov byte [kernel_error_code], 0x44
    mov eax, [si + 8]
    mov ebx, [si + 24]
    mov ecx, [si + 32]
    push si
...
.err:
    ...
    call print16
    mov si, msg_kernel_code
    call print16
    mov al, [kernel_error_code]
    call print_hex8
    mov si, msg_crlf
    call print16
    jmp halt_forever
...
.third_buffer:
    ...
    jmp .done
.overflow:
    mov byte [kernel_error_code], 0x60
    jmp load_kernel.err
.done:
    pop edx
    pop eax
...
msg_kernel_code    db '[ERR] Kernel code=0x', 0
